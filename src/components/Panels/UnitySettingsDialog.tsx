...
export default function UnitySettingsDialog({ open, onClose }: Props) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setMcpUrl(local || 'https://127.0.0.1:18082/mcp');
      setMessage('');
    }
  }, [open]);
  ...
  const handleSave = () => {
    const url = mcpUrl.trim() || 'https://127.0.0.1:18082/mcp';
        ...
        <div className="px-4 py-4 flex flex-col gap-4">
          <div>
            ...
            <input
              ...
              placeholder="https://127.0.0.1:18082/mcp"
            ...
            <div className="text-[12px] text-[#6c7086] mt-1">
              浏览器通过 HTTPS CORS 代理连接 Unity MCP（默认端口 18082）。
          ...
          <div className="bg-[#313244] rounded p-3">
            ...
            <div className="text-[12px] text-[#a6adc8]">
              ...
              <button
                onClick={() => {
                  const base = (mcpUrl.trim() || 'https://127.0.0.1:18082/mcp').replace(/\/mcp\/?$/, '');
                  window.open(base, '_blank');
                }}
                className="text-[#89b4fa] hover:underline bg-transparent border-none cursor-pointer p-0 font-mono text-[12px]"
              >
                {(mcpUrl.trim() || 'https://127.0.0.1:18082/mcp').replace(/\/mcp\/?$/, '')}
              </button>
