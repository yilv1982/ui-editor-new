...
**解决（2026-06-25）**：方向 1 落地——给离屏 working root 配置 NGUI 能识别的常驻 `anchorCamera`（挂 `UICamera`、对齐 CaptureLayer），widget 可见性判断通过、几何正常填充。配合 `UIDrawCall.cs` 源码改动（drawcall 出生跟随 manager scene）和 session 私有 PreviewScene 隔离，NGUI 常驻实例可正常渲染截图与 bbox，`DD_FP_HeroDisplay` open/render/close 通过，主工程无污染，`npm run smoke:ngui-snapshot` 通过（最大 bbox 漂移 0）。后续 NGUI 属性栏完整分支、mixed 画板、拖动节流真值回填见 `ngui-rewrite-plan.md` Step 7-8。
